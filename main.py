...
def absra_search(start, goal, avoid_nodes=None, penalty_nodes=None, penalty_factor=1.0, weights=None, absra_state=None):
    """
        ...
        if fwd_min <= bwd_min:
            ...
            for v in GRAPH_CONNECTIONS.get(u, []):
                ...
                if goal_region and v != goal and goal_region not in fwd_flags.get((u, v), {goal_region}):
                    continue
                base_cost = _edge_weight(_w, u, v)
                if penalty_nodes and v in penalty_nodes:
                    base_cost *= penalty_factor
                new_g = fwd_g[u] + base_cost
                if new_g < fwd_g[v]:
                    fwd_g[v] = new_g
                    fwd_par[v] = u
            ...
            for v in rev_adj.get(u, []):
                ...
                if start_region and v != start and start_region not in bwd_flags.get((v, u), {start_region}):
                    continue
                base_cost = _edge_weight(_w, v, u)
                if penalty_nodes and v in penalty_nodes:
                    base_cost *= penalty_factor
                new_g = bwd_g[u] + base_cost
                if new_g < bwd_g[v]:
                    bwd_g[v] = new_g
                    bwd_par[v] = u
...
def create_app():
    ...
    def register():
        ...
        if request.method == 'POST':
            ...
            if role == "Responder":
                ...
                new_team = Team(team_id=f"TM-{team_count:03d}", name=f"TM-{team_count:03d}", leader=f"{first_name} {last_name}", role="Response Unit", lat=lat, lon=lon, status="Available") # type: ignore
    ...
    def add_constraint():
        ...
        if node in NODE_LOCATIONS:
            ...
            if not existing:
                new_const = RoadConstraint(node_name=node, reason=reason, is_active=True) # type: ignore
    ...
    def calculate_route():
        ...
        has_light_vehicle = True # default to safest constraint if no vehicle is known
        if task_id != 'Manual':
            report = DisasterReport.query.filter_by(task_id=task_id).first()
            if report and report.resources:
                light_types = {'Ambulances', 'Vans', 'MPVs/Sedans'}
                # parse "Ambulances x1, Food Trucks x2"
                assigned_types = [r.split(' x')[0].strip() for r in report.resources.split(',') if ' x' in r]
                if assigned_types:
                    has_light_vehicle = any(t in light_types for t in assigned_types)
        ...
        for c in active_constraints:
            if c.constraint_type == 'road_block':
                avoid_nodes.append(c.node_name)
            elif c.constraint_type == 'heavy_traffic':
                penalty_nodes.add(c.node_name)
            elif c.constraint_type == 'flooded_road':
        ...
        # --- NEW UI CONSTRAINT ---
        modified_w = dict(base_w)
        penalty_factor = 5.0 # 5x time penalty for passing through soft constraints
        
        # Parse the obstacle coordinate sent from the UI
        ui_pin_node = None
        ...
        if constraint and baseline_path and len(baseline_path) > 1:
            want_alternative = True
            
            if constraint == 'heavy_traffic':
                if ui_pin_node:
                    penalty_nodes.add(ui_pin_node)
                else:
        ...
        if use_absra:
            ...
            path, _ = absra_search(
                start_node, end_node,
                avoid_nodes=avoid_nodes, penalty_nodes=penalty_nodes, penalty_factor=penalty_factor, weights=routing_w,
        ...
        if not path:
            failed_log = RouteLog(task_id=task_id, origin=start_node, destination=end_node,
                                  new_distance=0.0, reason=", ".join(avoid_nodes), status="Error") # type: ignore
        ...
        success_log = RouteLog(task_id=task_id, origin=start_node, destination=end_node,
                               new_distance=dist_km, reason=log_reason, status="Success") # type: ignore
    ...
    def update_status():
        ...
        if r and new_status:
            ...
            if new_status == "En Route":
                db.session.add(Notification(task_id=r.task_id, message=f"{r.task_id} — Team is En Route to {r.location}. Update status when task is complete.")) # type: ignore
                db.session.commit()
            elif new_status == "Waiting for Approval":
                db.session.add(Notification(task_id=r.task_id, message=f"{r.task_id} — Responder has completed task at {r.location} and is waiting for operator approval.")) # type: ignore
                db.session.commit()
    ...
    with app.app_context():
        ...
        for vtype in default_vehicles:
            if not VehicleResource.query.filter_by(vehicle_type=vtype).first():
                db.session.add(VehicleResource(vehicle_type=vtype, total=10, currently_assigned=0)) # type: ignore
        ...
        for u in users:
            if not User.query.filter_by(account_id=u["id"]).first():
                db.session.add(User(account_id=u["id"], password="password123", role=u["role"], first_name=u["fname"], last_name=u["lname"])) # type: ignore
        db.session.commit()
